...
#defining Pearson type IV distribution
#here x stands for temperature
def pearsoniv(x, kk0, kk1, kk2, kk3, kk4, kk5, kk6):
    return kk0 + kk1*(1+((x-kk6-kk2)/kk3)**2)**(-kk4)*np.exp(-kk5*(np.arctan(((x-kk6-kk2)/kk3)) + np.arctan(kk5/(2*kk4))))/(1+kk5**2/(4*kk4**2))**(-kk4)
...
c_pearson = np.zeros([Nline_new,Ncol]) 

#sweeping in magnetic fields
for j in range(Ncol):
    #fitting parameters
    par, par_cov = optimize.curve_fit(pearsoniv, T_expp[:], c_expp[:,j], maxfev=7000)
    ...
    for i in range(Nline_new):
        x  = T_expp[i]
        c_pearson[i,j] = k0[j] + k1[j]*(1+((x-k6[j]-k2[j])/k3[j])**2)**(-k4[j])*np.exp(-k5[j]*(np.arctan(((x-k6[j]-k2[j])/k3[j])) + np.arctan(k5[j]/(2*k4[j]))))/(1+k5[j]**2/(4*k4[j]**2))**(-k4[j])
...
if comparison_original_fitted == "y":
    ...
    for i in range(len(c_expp[0])):
        plt.scatter(T_expp[:], c_expp[:,i], label='curve', color=cmap(i))
    plt.plot(T_expp[:], c_pearson[:,:], label='curve', color=cmap(i+1))
    plt.xlabel('T (K)')
    plt.ylabel('c (J/kgK)')
    plt.show()
...
for i in range(Nline_new):
    for j in range(Ncol_new):
        ...
        c_fit[i,j] = kk0 + kk1*(1+((x-kk6-kk2)/kk3)**2)**(-kk4)*np.exp(-kk5*(np.arctan(((x-kk6-kk2)/kk3)) + np.arctan(kk5/(2*kk4))))/(1+kk5**2/(4*kk4**2))**(-kk4)
...
#Why using @ the highest temperature?
#Because at the highieste temperature, we have the paramagnetic phase where demagnetizing fields are less important, impacting less on the results

#defining vectors
Tlines = 11 #number of lines in M0 files
...
for j in range(Ncol):
    Mmax[j] = M[0,j]
    Mmin[j] = MM0[j] #Mmin have the same values as M0 (lowest magnetization values)
    for i in range(Nline_new):
        Mad[i,j] = (M[i,j]-Mmin[j])/(Mmax[j]-Mmin[j])
        Minv[i,j] = mp.sech(np.exp(np.pi/2 * Mad[i,j]))
...
def GaussCum(x, aa0, aa1, aa2, aa3):
    return aa0 + (aa1*np.arctan(((x-aa2)/aa3)+(np.pi/2)))/np.pi
...
for j in range(Ncol):
    par, par_cov = optimize.curve_fit(GaussCum, T_fit[:], Minv[:,j], maxfev=7000, p0=[0.3,0.6,270,1])
...
for i in range(Nline_new):
    for j in range(Ncol_new):
        ...
        #Calculating the equation (EquilConc) values to all fields
        eq[i,j] = aa0 + (aa1*np.arctan(((x-aa2)/aa3)+(np.pi/2)))/np.pi
        #Returing to Magnetization values for all fields - the following equation stands to the inversion fuction for sech
        M_fit[i,j] = (2/np.pi)*(np.log(np.log(np.sqrt(1/eq[i,j] -1)*np.sqrt(1/eq[i,j] + 1) + 1/eq[i,j])))*(MMmax-MMmin) + MMmin
...
plt.figure(figsize=(12, 8))
plt.plot(T_fit[:], dS_MxT[:,:], label='curva', color='red')
plt.plot(T_fit[:], ds[:,j], label='curva', color='black')
plt.scatter(T_exp[:], dS_exp[:,3], label='exp', color='blue')
plt.xlabel('T (K)')
plt.ylabel('ds (J/kgK)')
plt.show()
